...
<html>
<head>
<title><bc:message key='notification.viewedNotification'/></title>
...
function query(){
	var form1 = document.getElementById('form1');
	form1.action="<%=request.getContextPath() + IPageNavigation.INSTANCE.getPageURL(Constants.NOTIFICATION_FUNCTION, "queryViewedNotification.do")%>";
	form1.submit();
}
</script>
</head>
...
<table border="0" cellpadding="0" cellspacing="0" class="workarea" width="100%">
  <tr>
    <td class="workarea_title"><bc:message key='myTask.client'/> &gt;
        <h3><bc:message key='notification.viewedNotification'/></h3>
    </td>    
  </tr>
  <tr>
    <td>
      ...
      <table border="0" class="EOS_panel_body" width="100%">
        ...
        <tr>
          <td>
            ...
            <input type="hidden" name="showTitle" value="<bc:message key="notification.viewedNotification"/>" />
              ...
              <tr class="result_bottom">
                <td colspan="10" class="result_bottom" align="right">
                	<c:if test="${flag eq 'page'}">              		
						<c:set var="action" value='<%=request.getContextPath() + IPageNavigation.INSTANCE.getPageURL(Constants.NOTIFICATION_FUNCTION, "viewedNotificationList.do")%>'/>
					</c:if>		
                	<c:if test="${flag eq 'query'}">
						<c:set var="action" value='<%=request.getContextPath() + IPageNavigation.INSTANCE.getPageURL(Constants.NOTIFICATION_FUNCTION, "queryViewedNotification.do")%>'/>
					</c:if>
