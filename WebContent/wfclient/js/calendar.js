...
//cal = new CustomCalendar(by, ey,1,strFormat); //初始化英文版，0 为中文版 
cal = (cal==null) ? new CustomCalendar(by, ey, 0) : cal; //不用每次都初始化  修正 
...
//返回所选日期 
CustomCalendar.prototype.ReturnDate = function(dt) { 
...
* @update 
*/ 
function CustomCalendar(beginYear, endYear, lang, dateFormatStyle) { 
this.beginYear = 1950; 
this.endYear = 2050; 
...
mvAry[mvAry.length] = ' <tr>'; 
for(var i = 0; i < 7; i++) { 
mvAry[mvAry.length] = ' <th style="font-weight:normal;background-color:' + calendar.colors["tr_bg"] + ';color:' + calendar.colors["tr_word"] + ';">' + CustomCalendar.language["weeks"][this.lang][i] + '</th>'; 
} 
mvAry[mvAry.length] = ' </tr>'; 
for(var i = 0; i < 6;i++){ 
...
mvAry[mvAry.length] = ' <select name="calendarHour" id="calendarHour" style="font-size:12px;"></select>' + CustomCalendar.language["hour"][this.lang]; 
mvAry[mvAry.length] = '<span style="' 
if(calendar.DateMode >= pickMode["hour"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到小时时隐藏“分” 
mvAry[mvAry.length] = '"><select name="calendarMinute" id="calendarMinute" style="font-size:12px;"></select>' + CustomCalendar.language["minute"][this.lang]+'</span>'; 
mvAry[mvAry.length] = '<span style="' 
if(calendar.DateMode >= pickMode["minute"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到小时、分时隐藏“秒” 
mvAry[mvAry.length] = '"><select name="calendarSecond" id="calendarSecond" style="font-size:12px;"></select>'+ CustomCalendar.language["second"][this.lang]+'</span>'; 
...
mvAry[mvAry.length] = ' <input name="calendarClear" type="button" id="calendarClear" value="' + CustomCalendar.language["clear"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer;"/>'; 
mvAry[mvAry.length] = ' <input name="calendarToday" type="button" id="calendarToday" value="' 
mvAry[mvAry.length] = (calendar.DateMode == pickMode["day"]) ? CustomCalendar.language["today"][this.lang] : CustomCalendar.language["pickTxt"][this.lang]; 
...
mvAry[mvAry.length] = ' <input name="calendarClose" type="button" id="calendarClose" value="' + CustomCalendar.language["close"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer"/>'; 
...
//年份下拉框绑定数据 
CustomCalendar.prototype.bindYear = function() { 
var cy = this.calendarYear; 
cy.length = 0; 
for (var i = this.beginYear; i <= this.endYear; i++){ 
cy.options[cy.length] = new Option(i + CustomCalendar.language["year"][this.lang], i); 
} 
} 

//月份下拉框绑定数据 
CustomCalendar.prototype.bindMonth = function() { 
var cm = this.calendarMonth;
cm.length = 0; 
for (var i = 0; i < 12; i++){ 
cm.options[cm.length] = new Option(CustomCalendar.language["months"][this.lang][i], i); 
} 
} 

//小时下拉框绑定数据 
CustomCalendar.prototype.bindHour = function() { 
...
//分钟下拉框绑定数据 
CustomCalendar.prototype.bindMinute = function() { 
...
//秒钟下拉框绑定数据 
CustomCalendar.prototype.bindSecond = function() { 
...
//向前一月 
CustomCalendar.prototype.goPrevMonth = function(e){ 
...
//向后一月 
CustomCalendar.prototype.goNextMonth = function(e){ 
...
//改变SELECT选中状态 
CustomCalendar.prototype.changeSelect = function() { 
var cy = this.calendarYear;
var cm = this.calendarMonth; 
var ch = this.calendarHour; 
var cM = this.calendarMinute; 
...
//更新年、月 
CustomCalendar.prototype.update = function (e){ 
...
//绑定数据到月视图 
CustomCalendar.prototype.bindData = function () { 
var calendar = this; 
...
//根据年、月得到月视图数据(数组形式) 
CustomCalendar.prototype.getMonthViewArray = function (y, m) { 
var mvArray = []; 
...
CustomCalendar.prototype.getElementById = function(id){ 
...
CustomCalendar.prototype.getElementsByTagName = function(object, tagName){ 
if (document.getElementsByTagName) return document.getElementsByTagName(tagName); 
if (document.all) return document.all.tags(tagName); 
} 

//取得HTML控件绝对位置 
CustomCalendar.prototype.getAbsPoint = function (e){ 
var x = e.offsetLeft; 
var y = e.offsetTop; 
...
//显示日历 
CustomCalendar.prototype.show = function (dateObj, popControl) { 
if (dateObj == null){ 
...
//隐藏日历 
CustomCalendar.prototype.hide = function() { 
...
//焦点转移时隐藏日历  
CustomCalendar.prototype.onblur = function() { 
