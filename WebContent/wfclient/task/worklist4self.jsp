...
<%
	User user = new User();
	user.setUserId(UserDataContext.current().getUser().getUserID());
 	BpsQueryUtil bpsUtil = new BpsQueryUtil(user, null, 0, 100);
 	List<WFWorkItem> list = (List<WFWorkItem>)(request.getAttribute("tasklist"));
 	Map<Long, String> map = bpsUtil.getRelativeDataBatch(list,"param1");
%>
<html>
<head>
...
//*** 跳转页面 ***// 
function doGoto(processInstID){	
	var jsmap={};
	<%for(WFWorkItem item:list) { 
	String isEnd = "";
	if(item.getActivityDefID().endsWith("_END")){
		isEnd="&isEND=1";
	}
	%>	
	jsmap["<%=item.getProcessInstID()%>"]=
		"<%=map.get(item.getProcessInstID())%>?pID=<%=item.getProcessInstID()%>&wID=<%=item.getWorkItemID()%>&aID=<%=item.getActivityDefID()%>&aInstID=<%=item.getActivityInstID()%><%=isEnd%>";
	<%}%>
	
	var url = jsmap[processInstID];
...
function showDetail(id){
	var form1 = document.getElementById('form1');
	form1.action="<%=request.getContextPath()%>/showFlow.jsp?showBackButton=1&procInstID="+id;
    form1.workitemID.value=id;
    form1.showType.value="exe";
	form1.submit();
}
...
<table border="0" cellpadding="0"  align="center" cellspacing="0" class=" " width="90%">
  <tr>
    <td class=" "> &nbsp;  
  ...
  <tr>
    <td>
      <table border="0" align="center" class=" " width="100%">
        <tr>
          <td class=" "><!--bc:message key='myTask.QConditions'/-->&nbsp;</td>
        </tr>
        <tr>
          <td width="100%">
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0" class=" ">
              <tr align="left">
                <td width="15%" class=" "><bc:message key='myTask.exePer'/>:</td>
                <td align="left"  >
                    <input type="checkbox" id="chkAll1" name="chkAll1" value="ALL" class="" onClick="doCheckAll1(this);"> 
                    <bc:message key='myTask.all'/> &nbsp;&nbsp;
                    <input type="checkbox" id="private" name="private" value="PRIVATE" class="" onClick="doSelect1(this,2);"> 
                    <bc:message key='myTask.personal'/> &nbsp;&nbsp;
                    <input type="checkbox" id="public" name="public" value="PUBLIC" class="" onClick="doSelect1(this,1);"> 
                    <bc:message key='myTask.public'/> 
                </td>
                <td></td>
              </tr>
              <tr align="left">
                <td width="15%" class=" "><bc:message key='myTask.source'/>:</td>
                <td align="left">
        ...
        <tr>
          ...
          <form action="#" name="form1" id="form1" method="post">                
            ...
            <table border="0" cellpadding="0" cellspacing="0" width="100%">
              <tr>
                <td>
                  <table border="0" class="table table-condensed table-striped table-bordered table-hover" width="90%">
                    <tr class=" " align="center">
                       <th height="25" width="5%" nowrap="nowrap">流程图</th> <!-- bc:message key='myTask.Operation'/> </th> -->
                       <th height="25" width="5%" nowrap="nowrap"><bc:message key='myTask.WorkID'/></th>
                       <th height="25" width="5%" nowrap="nowrap"><bc:message key='myTask.WorkName'/></th>
                       <!-- th height="25" width="20%" nowrap="nowrap"><bc:message key='myTask.BPName'/></th> -->
                       <th height="25" width="15%" nowrap="nowrap"><bc:message key='myTask.BIName'/></th>
					...
					<c:forEach items="${tasklist}" var="task">
	                    <tr class=" " onmouseover="this.className=' '" onmouseout="this.className=' '">
  	                      	<td style="text-align:center;" nowrap> <a class="link" href='javascript:showDetail(${task.processInstID});'><bc:message key='myTask.View'/></a></td>
	                      	<td nowrap>${task.workItemID}</td>
	                      	<td nowrap>${task.workItemName}</td>
	                      	<!-- td nowrap>${task.processChName}(${task.processDefName})</td> -->
	                      	<td nowrap><a class="link" title='PI_${task.processInstID}|AI_${task.activityInstID}|WI_${task.workItemID}' href='javascript:doGoto("${task.processInstID}")'>${task.processInstName} </span></td>
	                      	<td nowrap><span class="label label-sm label-warning ng-binding"><bc:displayProInstCurrentState currentState='${task.currentState}' flag='task'/></span></td>
