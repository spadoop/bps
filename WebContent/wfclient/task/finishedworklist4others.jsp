...
<html>
<head>
<title><bc:message key='myTask.finished'/></title>
...
function doSelect2(chkObj,n){
    ...
    if(chkAll2.checked){
		...
		if(chkObj.checked){
			...
			if(n=="2"&&deleg.checked&&help.checked)
				chkAll2.checked=true;
			if(n=="3"&&agent.checked&&help.checked)
			    chkAll2.checked=true;
			if(n=="4"&&agent.checked&&deleg.checked)
			    chkAll2.checked=true;
	}
}
...
function query(){
	...
	if(chkAll2.checked)
	    bizPara = chkAll2.value;
	        ...
	        if(agent.checked&&deleg.checked&&help.checked)
	           bizPara = chkAll2.value;
	        if(bizPara==""){
    ...
    workItemFrm.action="<%=request.getContextPath() + IPageNavigation.INSTANCE.getPageURL(Constants.TASK_FUNCTION, "taskConditionQueryfinishedforSelf.do")%>";
...
<tr>
    <td class="workarea_title"><bc:message key='myTask.client'/> &gt; <h3><bc:message key='myTask.finished'/></h3> </td>
  </tr>
<tr>
<td>
  ...
  <tr> 
    <td width="100%">
	<!--查询条件显示开始-->
	<form action="#" id="workItemFrm" name="workItemFrm" method="post">
	<input type="hidden" id="bizPara" name="bizPara"></input>
            <table width="100%" border="0" cellpadding="0" cellspacing="0" class="form_table">
               <tr align="left">
                <td width="15%" class="EOS_table_row"><bc:message key='myTask.source'/>:</td><td align="left">
                   ...
                   <bc:message key='myTask.all'/>&nbsp;&nbsp;
                    ...
                    <input type="checkbox" id="agent" value="AGENT" class="checkbox" onClick="doSelect2(this,2);">
...
<table border="0" class="EOS_panel_body" width="100%">
  ...
  <tr> 
    ...
    <form action="#" name="form1" id="form1" method="post"> 
      ...
      <input type="hidden" name="showTitle" value="<bc:message key='myTask.finished'/>">
	...
	<tr>
		<td colspan="10" class="result_bottom" align="right">			
        	<c:if test="${flag eq 'default'}">
				<c:set var="action" value='<%=request.getContextPath() + IPageNavigation.INSTANCE.getPageURL(Constants.TASK_FUNCTION, "tasklistfinishedforSelf.do")%>'/>
			</c:if>
        	<c:if test="${flag eq 'condition'}">              		
				<c:set var="action" value='<%=request.getContextPath() + IPageNavigation.INSTANCE.getPageURL(Constants.TASK_FUNCTION, "taskConditionQueryfinishedforSelf.do")%>'/>
			</c:if>			
